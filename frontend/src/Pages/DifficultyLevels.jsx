      ...
      <div className="relative z-10 p-6 sm:p-8 max-w-6xl mx-auto min-h-screen">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center mb-12 gap-4">
          ...
          <div className="flex items-center gap-2 bg-white/10 backdrop-blur-lg border border-white/20 px-6 py-3 rounded-full text-white font-medium hover:bg-white/15 transition-all duration-300">
            ...
            <span className="capitalize">{continent} Capitals</span>  {/* Δείχνει την επιλεγμένη ήπειρο */}
          </div>
        </header>
        ...
        <section className="animate-slide-up animation-delay-400">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-6 max-w-6xl mx-auto">
            ...
            {difficultyOptions.map((option, index) => (
              <div
                key={option.id}
                className={`group relative bg-gradient-to-br ${option.bgColor} backdrop-blur-xl border-2 ${option.borderColor} ${option.hoverColor} rounded-3xl p-8 cursor-pointer transition-all duration-500 ease-out hover:-translate-y-4 hover:shadow-2xl hover:shadow-black/30 transform hover:scale-[1.02] ${
              ...
              >
                ...
                <div 
                  className={`absolute inset-0 rounded-3xl opacity-0 group-hover:opacity-25 transition-opacity duration-500 blur-xl bg-gradient-to-r ${option.color}`}
                ></div>
                ...
                <div className="relative z-10 text-center">
                  ...
                  {/* Difficulty Title */}
                  <h3 className={`text-2xl font-black mb-3 bg-gradient-to-r ${option.color} bg-clip-text text-transparent group-hover:scale-105 transition-transform duration-300`}>
                    {option.title}
                  </h3>
                  ...
                  {/* Points Badge */}
                  <div className={`inline-flex items-center gap-2 bg-gradient-to-r ${option.color} bg-opacity-20 border border-white/20 px-4 py-2 rounded-full text-white text-sm font-medium mb-6`}>
                    <span>⭐</span>
                ...
                <div className={`absolute inset-0 rounded-3xl border-2 border-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 bg-gradient-to-r ${option.color} p-[2px]`}>
                  <div className={`w-full h-full rounded-3xl bg-gradient-to-br ${option.bgColor} backdrop-blur-xl`}></div>
                </div>
              </div>
            ))}
          </div>
