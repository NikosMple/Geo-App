...
export default function GlobalMap() {
  ...
  // Preload geography data to ensure map works on first load
  useEffect(() => {
    fetch(GEO_URL)
  ...
  const resetZoom = () => setPosition({ coordinates: [0, 10], zoom: 1 });
  ...
  return (
    <div className="h-screen overflow-hidden app-background mounted p-4 md:p-8">
      ...
      <div className="max-w-[2000px] mx-auto relative z-10 h-full flex flex-col">
        <div className="mb-8">
          <Link
            to="/"
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-emerald-300/80 text-lg font-medium">
            Click countries • Scroll to zoom • Drag to pan • Press ESC to
            deselect
          </p>
        </div>
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between mb-3">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <option value="geoMercator" className="bg-slate-800 text-white">
                  Mercator
                </option>
              </select>
              <input
              ...
              <button
                ...
                className="px-3 py-2 rounded-lg text-xs font-medium bg-emerald-600 hover:bg-emerald-700 text-white transition-all"
              >
                Go
              </button>
              <button
                onClick={resetZoom}
                className="px-3 py-2 rounded-lg text-xs font-medium bg-white/5 text-orange-200 hover:bg-white/10 border border-white/10 transition-all"
              >
                Reset Zoom
              </button>
            </div>
        ...
        <div className="flex-1 min-h-0 relative">
          <div
            ref={containerRef}
            className="absolute inset-0 bg-white/5 backdrop-blur-lg rounded-xl shadow-2xl border border-white/10 overflow-hidden p-0"
          >
            {isLoading && (
            ...
            {!isLoading && !isError && geoData && (
              <div className="absolute inset-0 w-full h-full">
                <div className="absolute top-3 left-3 z-10 flex flex-col gap-2">
                  <button
                    onClick={zoomIn}
                    className="w-10 h-10 rounded-lg bg-white/10 text-white border border-white/10 hover:bg-white/20 text-xl font-bold"
                  >
                    +
                  </button>
                  <button
                    ...
                    <Geographies geography={geoData}>
                      {({ geographies }) =>
                        geographies.map((geo) => {
                          ...
                          const matchesFilter =
                            filterContinent === "all" ||
                            data?.continent === filterContinent;
                ...
                {hover.visible && (
                  <div
                    className="absolute z-20 px-3 py-2 rounded-md bg-black/80 backdrop-blur-sm border border-white/20 text-white text-xs pointer-events-none"
                  ...
                  >
                    ...
                    {countryData[hover.name] && (
                      <div className="opacity-90">
                        ...
                        <div>
                          Population: {countryData[hover.name].population}
                        </div>
                      </div>
                    )}
                  </div>
          ...
          {selectedCountry && !isLoading && !isError && (
            <div className="absolute top-4 right-4 w-80 max-h-[calc(100%-2rem)] bg-white/10 backdrop-blur-xl rounded-xl p-6 shadow-2xl border border-white/20 animate-slide-up overflow-y-auto z-20">
          ...
          {!selectedCountry && !isLoading && !isError && (
            <div className="absolute bottom-6 left-6 bg-black/30 backdrop-blur-sm rounded-lg px-4 py-3 border border-white/10 pointer-events-none z-10">
              <div className="flex items-center gap-2">
